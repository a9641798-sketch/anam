...
export default function SocialFeed() {
  ...
  const [feedImages, setFeedImages] = useState<string[]>(FEED_IMAGES);

  useEffect(() => {
    supabase?.from('site_settings').select('id, value').in('id', ['social_links', 'social_feed_images'])
      .then(({ data }) => {
        if (data) {
          const linksData = data.find(d => d.id === 'social_links');
          if (linksData?.value) {
            setSocialLinks({
              instagram: linksData.value.instagram || '#',
              pinterest: linksData.value.pinterest || '#'
            });
          }
          
          const imagesData = data.find(d => d.id === 'social_feed_images');
          if (imagesData?.value && Array.isArray(imagesData.value)) {
            // Merge with defaults so we always have 5 images even if some are empty
            const merged = imagesData.value.map((url, i) => url || FEED_IMAGES[i]);
            while (merged.length < 5) merged.push(FEED_IMAGES[merged.length]);
            setFeedImages(merged.slice(0, 5));
          }
        }
      });
  }, []);
